...
fn simple_line() {
    ...
    let result = d.find_path(0, 3, vec![1.0], &up_offset, &edges, true);
    assert!(result.is_some());
    let path = result.unwrap();
...
fn twice() {
    ...
    let result = d.find_path(0, 1, vec![1.0], &up_offset, &edges, true);
    assert!(result.is_some());
    let path = result.unwrap();
    assert_eq!(path.0, [0]);
...
fn multiple_paths() {
    ...
    let result = d.find_path(4, 4, vec![1.0], &up_offset, &edges, true);
    assert!(result.is_some());
    let path = result.unwrap();
    ...
    let result = d.find_path(6, 3, vec![1.0], &up_offset, &edges, true);
    assert!(result.is_some());
    let path = result.unwrap();
    assert_eq!(path.0, [7]);
    assert_eq!(path.1, 20.0);

    let result = d.find_path(1, 4, vec![1.0], &up_offset, &edges, true);
    assert!(result.is_some());
    let path = result.unwrap();
    assert_eq!(path.0, [2, 3, 4]);
    assert_eq!(path.1, 22.0);

    let result = d.find_path(0, 4, vec![1.0], &up_offset, &edges, true);
    assert!(result.is_some());
    let path = result.unwrap();
    assert_eq!(path.0, [1, 6, 9, 10, 11]);
    assert_eq!(path.1, 13.0);
}
...
fn ndim() {
    ...
    let result = d.find_path(0, 5, vec![0.0, 1.0], &up_offset, &edges, true);
    assert!(result.is_some());
    let path = result.unwrap();
